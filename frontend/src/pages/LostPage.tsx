...
// Actions available on the Lost tab
const lostActions: PatientAction[] = [
  {
...
export default function LostPage() {
  ...
  const handleAction = async (lead: Lead, action: PatientAction) => {
    // Re-engage: New Appointment → schedule modal
    ...
    // DNR — direct status update (DNRConfirmDialog handles confirmation in PatientCard)
    try {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="font-display text-2xl font-bold text-slate-900">Lost Patients</h1>
      ...
      {isLoading && leads.length === 0 && (
        <div className="flex items-center justify-center py-12">
          <div className="flex flex-col items-center gap-3">
            ...
            <p className="text-slate-500">Loading lost patients...</p>
          </div>
        </div>
      )}
      ...
      {!isLoading && leads.length === 0 && (
        <div className="flex flex-col items-center justify-center rounded-xl border-2 border-dashed border-slate-200 py-16">
          ...
          <h3 className="mt-4 font-semibold text-slate-900">No lost patients</h3>
          <p className="mt-1 text-sm text-slate-500">
            {searchQuery ? 'Try adjusting your search' : 'Patients appear here when marked as lost'}
          </p>
        </div>
      )}
